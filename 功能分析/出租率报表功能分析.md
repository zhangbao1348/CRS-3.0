...
### 4.4 数据生成
- generateDailyRooms：生成每日房量数据
- generateDailyRates：生成每日出租率数据
- generateGroupDailyData：生成全集团每日房量数据
- generateGroupDailyMaintenance：生成全集团每日维修房数据
- generateGroupDailySold：生成全集团每日已卖房数据
- generateGroupDailyOrders：生成全集团每日订单数数据
- generateGroupDailyRates：生成全集团每日出租率数据
...
### 4.6 行合并逻辑
- 按酒店纬度：每个酒店的5行数据（5种库存类型）合并为一行
- 按房型纬度：全集团的5行数据合并为一行，每个酒店的20行数据（4种房型 × 5种库存类型）合并为一行，每个房型的5行数据合并为一行
...
出租率报表是CRS系统中用于分析酒店出租率数据的功能模块。通过该页面，用户可以查看不同酒店、不同房型在特定月份的房量、维修房、已卖房、订单数和出租率等数据。

页面设计清晰，功能完整，交互流畅。用户可以通过选择酒店、统计方式和月份来筛选数据，通过表格查看详细的每日数据。

报表数据展示详细，包括酒店总房量、维修房、已卖房、订单数和出租率等关键指标，并按日期显示每日数据，帮助用户直观了解酒店的运营情况。

未来可以通过后端API获取实时出租率数据，实现更复杂的报表功能，如数据趋势分析、图表展示、自定义报表等。