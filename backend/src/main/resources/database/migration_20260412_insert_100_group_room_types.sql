...
-- 为租户2插入100个集团房型（房型大类ID: 6-10）
INSERT INTO group_room_types (group_id, room_type_code, room_type_name, description, room_type_category_id, max_occupancy, sort_order, status, created_at, updated_at) 
SELECT 
    2 AS group_id,
    CONCAT('GRP2_RT', LPAD(n, 3, '0')) AS room_type_code,
    SUBSTRING_INDEX(SUBSTRING_INDEX(@room_type_names, ',', n), ',', -1) AS room_type_name,
    CONCAT(SUBSTRING_INDEX(SUBSTRING_INDEX(@descriptions, ',', ((n - 1) % 24) + 1), ',', -1), '，编号', n) AS description,
    ((n - 1) % 5) + 6 AS room_type_category_id,
    CASE 
        WHEN n % 3 = 0 THEN 4
        WHEN n % 3 = 1 THEN 1
        ELSE 2
...
-- 为租户3插入100个集团房型（房型大类ID: 11-15）
INSERT INTO group_room_types (group_id, room_type_code, room_type_name, description, room_type_category_id, max_occupancy, sort_order, status, created_at, updated_at) 
SELECT 
    3 AS group_id,
    CONCAT('GRP3_RT', LPAD(n, 3, '0')) AS room_type_code,
    SUBSTRING_INDEX(SUBSTRING_INDEX(@room_type_names, ',', n), ',', -1) AS room_type_name,
    CONCAT(SUBSTRING_INDEX(SUBSTRING_INDEX(@descriptions, ',', ((n - 1) % 24) + 1), ',', -1), '，编号', n) AS description,
    ((n - 1) % 5) + 11 AS room_type_category_id,
    CASE 
        WHEN n % 3 = 0 THEN 4
        WHEN n % 3 = 1 THEN 1
        ELSE 2
...
-- 显示插入结果
SELECT CONCAT('集团房型数据初始化完成！共为 ', COUNT(DISTINCT group_id), ' 个租户插入 ', COUNT(*), ' 条集团房型数据') AS result 
FROM group_room_types;

-- 查看各租户的房型数量
SELECT group_id AS 租户ID, COUNT(*) AS 房型数量 
FROM group_room_types 
GROUP BY group_id 
ORDER BY group_id;
