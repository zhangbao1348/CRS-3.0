...
async function testHotelManagement() {
  ...
  try {
    ...
    for (let i = 0; i < hotels.length; i++) {
      ...
      try {
        // 根据前端代码的实际结构填写表单
        await page.type('input[placeholder="请输入酒店代码"]', hotel.hotelCode);
        await page.type('input[placeholder="请输入酒店中文名称"]', hotel.chineseName);
        await page.type('input[placeholder="请输入酒店英文名称"]', hotel.englishName);
        await page.type('input[placeholder="请输入酒店详细地址"]', hotel.address);
        await page.type('input[placeholder="请输入酒店联系电话"]', hotel.phone);
        await page.type('input[placeholder="请输入酒店邮箱"]', hotel.email);
        
        // 选择星级
        await page.click('div[role="listbox"]');
        const starOptions = await page.$$('div[role="option"]');
        for (const option of starOptions) {
          const text = await option.evaluate(el => el.textContent);
          if (text.includes(hotel.starRating === '5' ? '五级' : '四级')) {
            await option.click();
            break;
          }
        }
        ...
        // 选择省份
        await page.click('div[role="listbox"]');
        const provinceOptions = await page.$$('div[role="option"]');
        for (const option of provinceOptions) {
          const text = await option.evaluate(el => el.textContent);
          if (text.includes(hotel.province)) {
            await option.click();
            break;
          }
        }
        ...
        // 选择城市
        await page.click('div[role="listbox"]');
        const cityOptions = await page.$$('div[role="option"]');
        for (const option of cityOptions) {
          const text = await option.evaluate(el => el.textContent);
          if (text.includes(hotel.city)) {
            await option.click();
            break;
          }
        }
        ...
        // 填写总房间数
        await page.type('input[type="number"]', hotel.totalRooms);
