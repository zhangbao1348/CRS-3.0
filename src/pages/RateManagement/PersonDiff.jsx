...
const PersonDiff = () => {
  ...
  // 保存功能
  const handleSave = () => {
    console.log('保存人数差价数据:', pricingSystems)
    recordLog('保存', '保存了所有人数差价数据')
    alert('保存成功！')
  }
  ...
  const columns = [
    {
      ...
      render: (expanded, record) => {
        // 检查是否为原始数据行（id <= 7），原始行显示+号用于添加，新增行显示-号用于删除
        const isOriginalRow = record.id <= 7
          ...
          <Button
            ...
            onClick={() => {
              if (isOriginalRow) {
                ...
                // 记录日志
                recordLog('新增', `新增人数差价：${record.type}，值：${record.value}`)
              } else {
                // 删除当前行
                const updatedSystems = pricingSystems.map(system => {
                  if (system.id === activeSystemId) {
                ...
                // 记录日志
                recordLog('删除', `删除人数差价：${record.type}，值：${record.value}`)
              }
            }}
          />
        )
      ...
      render: (value, record) => (
        <Input
          ...
          onChange={(e) => {
            ...
            // 记录日志
            recordLog('修改', `修改人数差价：${record.type}，值：${oldValue} → ${newValue}`)
          }}
        />
      )
    },
      ...
      render: (startDate, record) => (
        <DatePicker
          ...
          onChange={(date) => {
            ...
            // 记录日志
            recordLog('修改', `修改人数差价：${record.type}，起始日期：${oldStartDate} → ${newStartDate}`)
          }}
          style={{ width: '100%' }}
        />
      )
      ...
      render: (endDate, record) => (
        <DatePicker
          ...
          onChange={(date) => {
            ...
            // 记录日志
            recordLog('修改', `修改人数差价：${record.type}，结束日期：${oldEndDate} → ${newEndDate}`)
          }}
          style={{ width: '100%' }}
        />
      )
      ...
      render: (weekdays, record) => (
        <Checkbox
          ...
          onChange={(e) => {
            ...
            // 记录日志
            recordLog('修改', `修改人数差价：${record.type}，适用星期：${oldWeekdays} → ${newWeekdays}`)
          }}
        />
      )
    },
