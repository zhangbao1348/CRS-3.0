...
const MarketLevelInventory = () => {
  ...
  const [selectedMarket, setSelectedMarket] = useState('全部市场')
  ...
  const filteredData = selectedMarket === '全部市场'
    ? tableData
    : tableData.filter(item => item.marketCode === selectedMarket)
      ...
      render: (_, record, index) => ({
        children: (
          <div style={{ fontSize: 12, display: 'flex', alignItems: 'center' }}>
            ...
            <span style={{ fontWeight: 600 }}>{record.marketName}</span>
          </div>
        ),
  ...
  const handleSubmit = () => {
    form.validateFields().then(values => {
      if (currentRow && currentDate) {
        message.success(`${currentRow.marketName} ${currentDate.dateStr} 库存限制已更新为 ${values.limit}`)
      } else {
        const marketNames = markets.filter(m => values.markets.includes(m.code)).map(m => m.name).join('、')
        message.success(`批量修改成功：${marketNames} 在所选日期范围的库存限制已更新为 ${values.limit}`)
      }
    ...
    <div>
      ...
      <Modal
        title={currentRow && currentDate 
          ? `维护 ${currentRow.marketName} - ${currentDate.dateStr} 库存限制` 
          : '批量修改库存限制'}
        open={isModalVisible}
        onOk={handleSubmit}
