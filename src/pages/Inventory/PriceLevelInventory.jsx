...
const PriceLevelInventory = () => {
  ...
  const [selectedPriceCode, setSelectedPriceCode] = useState('全部价格')
  ...
  const generateData = () => {
    ...
    priceCodes.forEach(priceCode => {
      inventoryTypes.forEach(invType => {
        const row = {
          key: `${priceCode.code}_${invType.key}`,
          priceCode: priceCode.code,
          priceName: priceCode.name,
  ...
  const filteredData = selectedPriceCode === '全部价格'
    ? tableData
    : tableData.filter(item => item.priceCode === selectedPriceCode)
      ...
      render: (_, record, index) => ({
        children: (
          <div style={{ fontSize: 12, display: 'flex', alignItems: 'center' }}>
            ...
            <span style={{ fontWeight: 600 }}>{record.priceName}</span>
          </div>
        ),
  ...
  const handleSubmit = () => {
    form.validateFields().then(values => {
      if (currentRow && currentDate) {
        message.success(`${currentRow.priceName} ${currentDate.dateStr} 库存限制已更新为 ${values.limit}`)
      } else {
        const priceNames = priceCodes.filter(pc => values.priceCodes.includes(pc.code)).map(pc => pc.name).join('、')
        message.success(`批量修改成功：${priceNames} 在所选日期范围的库存限制已更新为 ${values.limit}`)
      }
    ...
    <div>
      ...
      <Modal
        title={currentRow && currentDate 
          ? `维护 ${currentRow.priceName} - ${currentDate.dateStr} 库存限制` 
          : '批量修改库存限制'}
        open={isModalVisible}
        onOk={handleSubmit}
