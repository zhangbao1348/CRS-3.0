  ...
  const generateData = () => {
    ...
    channels.forEach(channel => {
      inventoryTypes.forEach(invType => {
        const row = {
          key: `${channel.code}_${invType.key}`,
  ...
  const handleSubmit = () => {
    form.validateFields().then(values => {
      if (currentRow && currentDate) {
        message.success(`${currentRow.channelName} ${currentDate.dateStr} 库存限制已更新为 ${values.limit}`)
      } else {
        const channelNames = channels.filter(c => values.channels.includes(c.code)).map(c => c.name).join('、')
        message.success(`批量修改成功：${channelNames} 在所选日期范围的库存限制已更新为 ${values.limit}`)
      }
    ...
    <div>
      ...
      <Modal
        title={currentRow && currentDate 
          ? `维护 ${currentRow.channelName} - ${currentDate.dateStr} 库存限制` 
          : '批量修改库存限制'}
        open={isModalVisible}
        onOk={handleSubmit}
