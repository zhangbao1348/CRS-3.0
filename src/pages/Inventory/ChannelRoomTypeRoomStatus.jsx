...
import { FilterOutlined, PlusOutlined, MinusOutlined, HomeOutlined, ApiOutlined } from '@ant-design/icons'

const { Option } = Select

const ChannelRoomTypeRoomStatus = () => {
  ...
  const roomTypes = [
    { code: '1KGS', name: '标准大床房' },
    { code: '1KGP', name: '高级大床房' },
    { code: '1KGH', name: '探索大床房' },
    { code: '2TGH', name: '探索双床房' }
  ]
  ...
  const generateData = () => {
    ...
    roomTypes.forEach(roomType => {
      channels.forEach(channel => {
        const row = {
          key: `${roomType.code}_${channel.code}`,
  ...
  const columns = [
    {
      ...
      render: (_, record) => (
        <div style={{ fontSize: 12, display: 'flex', alignItems: 'center' }}>
          ...
          <ApiOutlined style={{ marginLeft: 6, marginRight: 4, color: '#1890ff', fontSize: 12 }} />
          <span style={{ color: '#666' }}>{record.channelName}</span>
        </div>
      )
    },
    ...dates.map(date => ({
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'flex-start', alignItems: 'center', marginBottom: 16, gap: 16 }}>
        <Select value={selectedChannel} style={{ width: 180 }} size="middle" onChange={setSelectedChannel}>
...
export default ChannelRoomTypeRoomStatus