...
import { FilterOutlined, PlusOutlined, MinusOutlined, ApiOutlined } from '@ant-design/icons'

const { Option } = Select

const ChannelLevelRoomStatus = () => {
  ...
  const filteredData = selectedChannel === '全部渠道' 
    ? tableData 
    : tableData.filter(item => item.channelCode === selectedChannel)
  ...
  const columns = [
    {
      ...
      render: (_, record) => (
        <div style={{ fontSize: 12, display: 'flex', alignItems: 'center' }}>
          <ApiOutlined style={{ marginRight: 6, color: '#1890ff', fontSize: 14 }} />
          <span style={{ fontWeight: 600 }}>{record.channelName}</span>
        </div>
      )
    },
    ...dates.map(date => ({
...
export default ChannelLevelRoomStatus