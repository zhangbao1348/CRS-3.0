...
import { FilterOutlined, PlusOutlined, MinusOutlined, GlobalOutlined } from '@ant-design/icons'

const { Option } = Select

const MarketLevelRoomStatus = () => {
  ...
  const columns = [
    {
      ...
      render: (_, record) => (
        <div style={{ fontSize: 12, display: 'flex', alignItems: 'center' }}>
          <GlobalOutlined style={{ marginRight: 6, color: '#1890ff', fontSize: 14 }} />
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'flex-start', alignItems: 'center', marginBottom: 16, gap: 16 }}>
        <Select value={selectedMarket} style={{ width: 180 }} size="middle" onChange={setSelectedMarket}>
...
export default MarketLevelRoomStatus